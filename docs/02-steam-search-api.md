...
| `start` | Невыровненное значение игнорируется: `start=20&count=25` вернул первую страницу. С `count=100` страницы `start=100`, `200` и далее до `1100` работают и не пересекаются, проверено 12 сентября |
...
При `sort_by=Reviews_DESC` на первой странице гейт почти ничего не режет, потому что сортировка уже вывела лучшее вперёд. На следующих страницах всё иначе: там идут игры с 81–94% положительных, затем Positive с 10–49 отзывами и Mostly Positive с 70–79%. При нескольких страницах именно гейт решает, что уйдёт в чат, см. «Одна страница или несколько».

## Одна страница или несколько

Выдача с `sort_by=Reviews_DESC` упорядочена по категории отзывов, а не по проценту. Одна страница из 100 строк это только лучшие ~4% скидок, и порог отзывов ниже ~92% на ней не действует. Замер 12 сентября, `cc=kz`, итоговый запрос, пороги 70% / 80% / 200 отзывов:

| Страниц | Подпись отзывов на последней странице | Процент положительных | Прошло порог, всего |
|---|---|---|---|
| 1 | Overwhelmingly и Very Positive | 92–100 | 24 |
| 3 | Very Positive | 90–95 | 71 |
| 5 | Very Positive | 81–94 | 130 |
| 7 | Very Positive | 81–89 | 203 |
| 9 | Very Positive, первые Positive и Mostly Positive | 75–100 | 248 |
| 10–12 | Positive, 10–49 отзывов | 90–100 | 248 |

После Very Positive идут Positive, это игры с 10–49 отзывами, затем Mostly Positive с 70–79%. При пороге от 80% и от 50 отзывов дальше 9–10 страниц подходящих игр нет. В большую распродажу скидок больше, и граница сдвигается дальше.

Двенадцать запросов подряд с паузой 12 секунд прошли без 429. В боте число страниц задаёт `STEAM_MAX_PAGES`.

Одна страница создаёт ещё и дубли: игра на границе первой сотни выпадает из выдачи, когда в распродажу появляются игры с лучшими отзывами, забывается через `FORGET_AFTER_DAYS` и приходит снова, когда возвращается.
